...
4. **可验证**：每次加载时验证数据完整性
5. **容错隔离**：结构化文件与 API keys 文件写入独立 try-catch，一类文件失败不影响另一类
...
- API Key 在结构化状态文件中始终为空字符串（`stripProviderSecret` 处理）
- `persistSecrets()` 通过 `readProviderApiKeysFromDisk()` 从磁盘读取其他 IDE 的 keys 进行合并
- `reloadFromSharedStorage()` 在 flush 前保存磁盘 keys，防止本地 flush 覆盖其他 IDE 的新增 keys
- 合并策略：本地 keys 优先，磁盘独有的 keys 补入，内存快照独有的 keys 保留
